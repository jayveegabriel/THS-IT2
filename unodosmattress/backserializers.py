...
from simple_aes_cipher
import AESCipher, generate_secret_key

class TemperatureSerializer(serializers.HyperlinkedModelSerializer):
	pass_phrase = "unodosmattress"
	...
	def create(self, validated_data):
		...
		if str(tempTime) == "23:59:59":
			...
			todo = Temperature(
	        	temperature= cipher.decrypt(validated_data.get('temperature', '')),
	        	date=cipher.decrypt(date),
	        	time=cipher.decrypt(time),
	...
	def create(self, validated_data):
		...
		if str(tempTime) == "23:59:59":
			...
			if patient:
				...
				hr = HeartRate(
	        	heartRate=cipher.decrypt(validated_data.get('heartRate', ''),
	        	date=cipher.decrypt(date),
	        	time=cipher.decrypt(time),
				idPatient_id = 10,
				)
...
class TemperatureEveryMinSerializer(serializers.HyperlinkedModelSerializer):
	...
	def create(self, validated_data):
		...
		if str(tempTime) == "23:59:59":
			...
			todo = TemperatureEveryMinute(
	        	temperature= cipher.decrypt(validated_data.get('temperature', '')),
	        	date=cipher.decrypt(validated_data.get('date', '')),
	        	time=cipher.decrypt(validated_data.get('time', '')),
				idPatient_id = patient,
				)
			todo.save()
		return todo
